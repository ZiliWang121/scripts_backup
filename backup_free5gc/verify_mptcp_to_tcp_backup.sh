...
PKT_SUMMARY=$(sudo timeout $DURATION tcpdump -n -tt -i lo port $ECHO_PORT 2>/dev/null | wc -l)
PSH_COUNT=$(sudo timeout $DURATION tcpdump -n -i lo port $ECHO_PORT 2>/dev/null | grep -c "Flags \[P.")
