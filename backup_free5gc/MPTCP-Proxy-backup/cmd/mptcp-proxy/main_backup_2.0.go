...
const MPTCP_ENABLED = 42  // ← 使用你系统支持的值
...
// ✅ 完全兼容 IPv4，使用 syscall.RawSyscall 获取原始目的地址
func getOriginalDestination(sockfd int) (net.IP, int, error) {
	var addr [16]byte  // struct sockaddr_in 占 16 字节
	var size = uint32(len(addr))
	...
	if errno != 0 {
		return nil, 0, fmt.Errorf("getsockopt(SO_ORIGINAL_DST) failed: %v", errno)
	}
...
func handleConnection(fd int, src, dst *syscall.SockaddrInet4, connectProtocol int) error {
	...
	// ✅ 禁用 rFd 上的 MPTCP（用于连接外部 server）
	val := 0
	err = syscall.SetsockoptInt(rFd, syscall.IPPROTO_TCP, MPTCP_ENABLED, val)
	if err != nil {
		log.Warnf("Disabling MPTCP failed, maybe not supported: %v", err)
	}
